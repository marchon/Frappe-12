...
def total_count():
	return {
		"applicant" : frappe.db.count('Student Applicant'),
		"student"	: frappe.db.count('Student'),
		"batch"		: frappe.db.count('Student Group'),
		"fee"		: frappe.db.sql('select sum(received_amount) from `tabPayment Entry`')[0][0],
		"rec_pay"	: frappe.db.get_all('Payment Entry', fields=['name','party_name','received_amount','creation'], filters={'docstatus': 1}),
		"stu_list"	: frappe.db.get_all('Student', fields=['name','title','creation']),
		"instructor": frappe.db.get_all('Instructor', fields=['instructor_name','department','creation','employee']),
		"task"	: frappe.db.get_all('Daily Task', fields=['name','subject','exp_start_date','exp_end_date','status'])
	}
